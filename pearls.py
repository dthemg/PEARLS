...
def PEARLS(
	...
	frequency_matrix = np.arange(1, max_num_harmonics + 1) * pitch_candidates.reshape(
		(num_pitch_candidates, 1)
	)
		...
		if history_idx == 0:
			...
			if upper_time_idx - history_idx < history_len:
				time_history = np.append(
					time_history, np.zeros(
						history_len - (upper_time_idx - smaple_idx))
				)
